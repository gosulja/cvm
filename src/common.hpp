...
#define LDR      0x01 // load register with a value
...
inline std::string prettyBytecode(const std::vector<uint16_t>& bytecode) {
    ...
    while (i < bytecode.size()) {
        ...
        if (instructionMap.find(inst) != instructionMap.end()) {
            ...
            if (inst == LDR || inst == LOADV || inst == STOREV || inst == ADD ||
